...
test.describe('The Documentation page', () => {
  test('should render without breaking the app (server side rendered)', async ({ page }) => {
    await visit(page, '/documentation/bilan')

    expect(page.locator(`h1[data-cypress-id="${DOCUMENTATION_TITLE}"]`)).toBeDefined()
  })
  ...
  test('should render the client side documentation upon click on the launch button', async ({
    ...
    await page.locator(`button[data-cypress-id="${DOCUMENTATION_LAUNCH_BUTTON}"]`).click()
