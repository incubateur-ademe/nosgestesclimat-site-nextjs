...
import { useFetchDocumentation } from '@/app/(layout-with-navigation)/(simulation)/actions/_hooks/useFetchDocumentation'
...
export default function CategoryGuide({ category }: { category: string }) {
  ...
  const { data: documentationData } = useFetchDocumentation()

  if (!documentationData) {
    return <Loader size="md" color="dark" className="m-4" />
  }
