...
vi.mock('@/publicodes-state/helpers/getComputedResults', () => ({
  getComputedResults: () => ({ carbone: { bilan: 1000 } }),
}))

const mockUpdateSimulationSituation = vi.fn()
vi.mock('@/services/simulations/update-simulation-situation', () => ({
  updateSimulationSituation: (payload: unknown) =>
    mockUpdateSimulationSituation(payload),
}))
...
describe('useSaveSimulationProgress', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockUpdateSimulationSituation.mockResolvedValue({ success: true })
  ...
  it('should not save when the simulation changes without a request', async () => {
    ...
    expect(mockUpdateSimulationSituation).not.toHaveBeenCalled()
  })
  ...
  it('should save once when a request follows a change', async () => {
    ...
    expect(mockUpdateSimulationSituation).toHaveBeenCalledTimes(1)
    expect(mockUpdateSimulationSituation).toHaveBeenCalledWith(
  ...
  it('should serialize saves rather than let them race', async () => {
    ...
    expect(mockUpdateSimulationSituation).toHaveBeenCalledTimes(2)
  })

  it('should report a failed save to Sentry', async () => {
    const error = new Error('simulation_completed')
    mockUpdateSimulationSituation.mockResolvedValue({ success: false, error })
